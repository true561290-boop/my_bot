...
# --- 🌐 خادم ويب صغير وإرسال طلبات ذاتية للبوت نفسه ---
app = Flask('')
...
# --- 🔄 كود الزيارة الذاتية (يقوم البوت بزيارة نفسه كل 5 دقائق) ---
async def self_ping():
    await asyncio.sleep(10)  # انتظار ثوانٍ حتى يعمل خادم الويب أولاً
        ...
        while True:
            try:
                async with session.get(render_url) as resp:
                    print(f"🔄 [Self-Ping] تم زيارة البوت لنفسه بنجاح! كود الاستجابة: {resp.status}")
            except Exception as e:
                print(f"⚠️ [Self-Ping] خطأ أثناء المحاولة: {e}")
...
async def on_ready():
    ...
    # 💥 تشغيل حلقة الزيارة الذاتية لمنع النوم
